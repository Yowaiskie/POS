...
<div class="max-w-[1600px] mx-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        @forelse($rooms as $room)
            ...
            @endphp
            <div class="bg-white rounded-xl border-2 {{ $status !== 'available' ? "border-$statusColor-200" : 'border-slate-100' }} p-6 shadow-sm hover:shadow-lg transition-all duration-200 h-full flex flex-col group relative overflow-hidden">
                @if($status !== 'available')
                ...
                <div class="flex items-start justify-between mb-4 relative z-10">
                    <div class="flex-1">
                        ...
                        <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border 
                            @if($status === 'active') bg-emerald-50 text-emerald-700 border-emerald-200
                            @elseif($status === 'warning') bg-amber-50 text-amber-700 border-amber-200
                            @elseif($status === 'overtime') bg-rose-50 text-rose-700 border-rose-200
                            @else bg-slate-50 text-slate-600 border-slate-200 @endif uppercase tracking-wider">
                            <span class="w-2 h-2 rounded-full @if($status === 'active') bg-emerald-500 animate-pulse @elseif($status === 'warning') bg-amber-500 animate-bounce @elseif($status === 'overtime') bg-rose-500 animate-ping @else bg-slate-400 @endif"></span>
                            {{ $status }}
                        </span>
                    </div>
                </div>
                ...
                <div class="flex-1 flex flex-col justify-center py-6 bg-slate-50 rounded-xl mb-4 relative z-10">
                    <div class="text-center">
                        ...
                        <div class="font-mono text-3xl font-black @if($status === 'active') text-emerald-600 @elseif($status === 'warning') text-amber-600 @elseif($status === 'overtime') text-rose-600 @else text-slate-300 @endif tracking-tighter">
                ...
                <div class="grid grid-cols-1 gap-2.5 relative z-10">
                    ...
                    @else
                        <div class="flex gap-2">
                            <a href="{{ route('orders.index', ['room_id' => $room->id]) }}" class="flex-1 px-4 py-2.5 bg-white border-2 border-slate-200 text-slate-700 rounded-lg hover:border-indigo-600 hover:text-indigo-600 active:scale-95 transition-all text-sm font-bold text-center">
                                Orders
                            </a>
